...
class CameraPoseProvider(Dataset):
    ...
    @property
    def get_reso(self):
        index = bisect.bisect(self.reso_milestones, self.step + 1) - 1
    ...
    def get_azimuth_bound(self):
        if not self.stratified_on_azimuth:
            return [
                self.azimuth[0]
                * min(self.step + 1 / (self.azimuth_warmup * self.max_steps + 1e-5), 1.0),
                self.azimuth[1]
                * min(self.step + 1 / (self.azimuth_warmup * self.max_steps + 1e-5), 1.0),
            ]
        else:
    ...
    def get_elevation_bound(self):
        return [
            self.elevation[0]
            * min(self.step + 1 / (self.elevation_warmup * self.max_steps + 1e-5), 1.0),
            self.elevation[1]
            * min(self.step + 1 / (self.elevation_warmup * self.max_steps + 1e-5), 1.0),
        ]
    ...
    def get_focal_bound(self):
        index = bisect.bisect(self.focal_milestones, self.step + 1) - 1
