...
from utils.ops import binary_cross_entropy, marching_cubes
from utils.initialize import base_initialize, initialize
from utils.dpt import DPT
from utils.export import get_density_val_grid_from_ckpt
...
class Trainer(nn.Module):
	def __init__(self, cfg):
		...
		normalize_scene()
		bpy.ops.wm.obj_export(filepath=f"{cfg.guidance.control_obj_uid}.obj")
	...
	def visualizieImages(self):
		...
		final = (out['rgb'].cpu().detach().numpy() * 255.0).astype(np.uint8)

		imageio.imwrite("test.png", final)

	def get_control_image_from_mesh(self, cfg, batch, mesh):
		R, T = look_at_view_transform(dist=batch["camera_distance"].to(torch.float32),
		...
		cameras = FoVPerspectiveCameras(device=cfg.device,
										...
										znear=torch.tensor(
											list(map(lambda camera_info: camera_info.near_plane, batch["camera_info"]))),
										zfar=torch.tensor(
											list(map(lambda camera_info: camera_info.far_plane, batch["camera_info"]))),
										aspect_ratio=torch.tensor(
											list(map(lambda camera_info: camera_info.aspect, batch["camera_info"]))),
										fov=torch.tensor(list(map(lambda camera_info: camera_info.yfov, batch["camera_info"]))))
		...
		lights = PointLights(device=cfg.device, location=batch["light_pos"])
		...
		return list(map(functional.to_pil_image, three_channel_depth.permute(0, 3, 1, 2)))
	...
	def get_mesh_from_gaussians(self, cfg):
		...
		# print(torch.from_numpy(vertices).cuda().shape)
		# save_obj("represent.obj", verts=torch.from_numpy(vertices).cuda(),
		# 				faces=torch.from_numpy(triangles.astype(np.int64)).cuda())
		# exit()
	...
	def dgm_loss_step(self):
		...
		print("gt_3dgm: ", gt_3dgm)
		print("intermediate_3dgm: ", intermediate_3dgm)
		mse_loss = nn.MSELoss()
	...
	def train_step(self):
		...
		if self.cfg.guidance.controlled:
			self.control_image = self.get_control_image_from_mesh(
		...
		if "loss_sds" in guidance_out.keys():
			...
			self.writer.add_scalar(
				"loss/sds", guidance_out["loss_sds"], self.step)
		...
		if "loss_vsd" in guidance_out.keys():
			...
			self.writer.add_scalar(
				"loss/vsd", guidance_out["loss_vsd"], self.step)
		...
		if "loss_lora" in guidance_out.keys():
			...
			self.writer.add_scalar(
				"loss/lora", guidance_out["loss_lora"], self.step)
		...
		if self.cfg.loss.sparsity > 0.0:
			...
			loss += C(self.cfg.loss.sparsity, self.step,
					  self.max_steps) * sparsity_loss
			self.writer.add_scalar(
		...
		if self.cfg.loss.opague > 0.0:
			...
			opacity_loss = binary_cross_entropy(
				opacity_clamped, opacity_clamped)
			self.writer.add_scalar("loss/opague", opacity_loss, self.step)
			loss += C(self.cfg.loss.opague, self.step,
					  self.max_steps) * opacity_loss
			self.writer.add_scalar(
				"loss_weights/opague",
		...
		if self.cfg.loss.z_var > 0:
			...
			loss += C(self.cfg.loss.z_var, self.step,
					  self.max_steps) * z_var_loss
			self.writer.add_scalar(
				"loss_weights/z_var", C(self.cfg.loss.z_var,
		...
		with torch.no_grad():
			if step_check(self.step, self.cfg.log_period, run_at_zero=True):
				...
				if "depth" in out.keys():
					...
					out["depth"] = apply_depth_colormap(
						out["depth"], out["opacity"])
				...
				final = (
					torch.cat(list(out.values()), dim=-
							  2).clamp(0, 1).cpu().numpy()
					* 255.0
				).astype(np.uint8)[-1]
				imageio.imwrite(
					str(train_image_pth / f"{self.step}.png"), final)
				self.writer.add_image(
					"train/image", final, self.step, dataformats="HWC"
				)
				# self.writer.add_image(
				#     "train/gms", self.control_image[0], self.step, dataformats="HWC"
				# )
	...
	def estimator_loss_step(self, out):
		...
		if self.cfg.estimators.depth.enabled:
			...
			self.writer.add_scalar(
				"loss/depth", depth_estimate_loss, self.step)
			depth_loss_weight = C(
				self.cfg.estimators.depth.value, self.step, self.max_steps
			)
			self.writer.add_scalar("loss_weights/depth",
								   depth_loss_weight, self.step)
		...
		if self.cfg.estimators.normal.enabled:
			...
			self.writer.add_scalar(
				"estimator_loss/normal", normal_estimator_loss)
	...
	def aux_guidance_step(self):
		if self.cfg.auxiliary.enabled:
			...
			self.writer.add_scalar("loss/aux_guidance",
								   aux_guidance_loss, self.step)
			loss = (
	...
	def eval_image_step(self):
		...
		self.writer.add_image("eval/image", final,
	...
	def eval_video_step(self):
		...
		if "depth" in outs.keys():
			...
			outs["depth"] = apply_depth_colormap(
				outs["depth"], outs["opacity"])
			outs["opacity"] = apply_float_colormap(outs["opacity"])

		if "z_var" in out.keys():
			outs["z_var"] = outs["z_var"] / \
				outs["opacity"] * (outs["opacity"] > 0.5)
			outs["z_var"] = apply_float_colormap(
				outs["z_var"] / outs["z_var"].max())
		...
		with tqdm(total=self.max_steps - self.start) as pbar:
			for s in range(self.start, self.max_steps):
				...
				pbar.set_description(
					f"{self.timestamp}|Iter: {s}/{self.max_steps}")
	...
	def train_step_sit3d(self):
		...
		if "loss_sds" in guidance_out.keys():
			...
			self.writer.add_scalar(
				"loss/clip", guidance_out["loss_clip"], self.step)
		else:
		...
		if num_original_views > 0:
			...
			loss += C(self.cfg.loss.image, self.step,
					  self.max_steps) * image_loss
			...
			loss += C(self.cfg.loss.depth, self.step,
					  self.max_steps) * depth_loss_val
	...
	def tune_with_upsample_model(self):
		...
		if not self.cfg.upsample_tune.use_cache or not cache_tmp_file.exists():
			console.print(
				f"[green]no cache found, will save to {str(cache_tmp_file)}")
		...
		with tqdm(total=max_steps) as pbar:
			for e in range(epoch):
				for i in range(0, total, bs):
					...
					if self.cfg.upsample_tune.loss.sds > 0.0:
						loss += (
							self.cfg.upsample_tune.loss.sds *
							guidance_out["loss_sds"]
						)
					...
					pbar.set_description(
						f"Upsample Tune|Iter: {self.step}/{max_steps}")
					pbar.set_postfix(loss=f"{loss:.4f}")
